function dataset = Dataset_test_ss(dataset, dataset_name, usage, use_flip)
...
switch usage
    case {'train'}
        dataset.imdb_train    = {  imdb_from_voc(devkit, dataset_name, 'test', '2007', use_flip) };
        dataset.roidb_train   = cellfun(@(x) x.roidb_func(x, 'with_selective_search', true), dataset.imdb_train, 'UniformOutput', false);
    case {'test'}
        dataset.imdb_test     = imdb_from_voc(devkit, dataset_name, 'test', '2007', use_flip) ;
